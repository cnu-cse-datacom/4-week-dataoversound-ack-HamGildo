...
def match_num(byte_stream):
    mynum = bytearray(b'201702087')
    for i in byte_stream:
        if i == 50:
           start = byte_stream.index(i)
           if start+9 < len(byte_stream):
               byte_stream2 = byte_stream[start:start+9]
               if mynum == byte_stream2:
                   return True
    return False  
...
def make_freq(byte_stream, sin_freq):
    sin_freq.append(HANDSHAKE_START_HZ)
    for i in byte_stream:
        sin_freq.append((i >> 4) * STEP_HZ + START_HZ)
        sin_freq.append((i & 0xf) * STEP_HZ + START_HZ)
    sin_freq.append(HANDSHAKE_END_HZ)
    return sin_freq

def make_sound(sin_freq):
    ...
    for i in sin_freq:
         print(i)
         signal = np.sin(2*np.pi*i*t).astype(np.float32)
         stream.write(signal)
    stream.stop_stream()
    stream.close()
    p.terminate()
